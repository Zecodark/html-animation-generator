...
import { TsxRenderEngine } from "@/lib/renderer/TsxRenderEngine";
import { RenderCancelledError } from "@/lib/renderer/RenderEngine";
import { Throttled } from "@/lib/renderer/RenderScheduler";
import { downloadBlob } from "@/lib/utils/utils";
import type { TsxFrameCapture } from "@/lib/renderer/TsxFrameCapture";

/**
 * TSX-specific renderer hook.
 *
 * Uses TsxRenderEngine (with periodic iframe memory purging) instead of
 * the standard RenderEngine. This is SEPARATE from useRenderer (HTML)
 * so that changes to the TSX pipeline never risk breaking the HTML pipeline.
 */
export function useTsxRenderer(frameCaptureRef: RefObject<TsxFrameCapture | null>) {
  ...
  const renderJob = useCallback(
    async (jobId: string): Promise<boolean> => {
      ...
      try {
        ...
        // Use TsxRenderEngine instead of RenderEngine
        const engine = new TsxRenderEngine(frameCapture, selected.adapter);
